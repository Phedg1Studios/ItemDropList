    ...
    namespace ItemDropList {
        public class ButtonCreator : MonoBehaviour {
            ...
            static public RectTransform SpawnItemButton(GameObject root, int textCount, int givenID, Dictionary<int, List<Image>> images, Dictionary<int, List<TMPro.TextMeshProUGUI>> texts, bool isFallback = false) {
                ...
                } else if (Data.allDroneIDs.ContainsKey(givenID)) {
                    itemImage = Data.allDroneIcons[givenID];
                }
                ...
                */
                ...
                ElementCreator.SpawnImageOffset(images[givenID], scaler, null, new Color(0, 0, 0, 0.85f), new Vector2(0, 1), new Vector2(0, 0), new Vector2(0, 0));
                images[givenID][images[givenID].Count - 1].raycastTarget = true;
                images[givenID][images[givenID].Count - 1].gameObject.SetActive(false);
                ElementCreator.SpawnImageSize(images[givenID], scaler, Resources.panelTextures[10], new Color(1, 1, 1, 0.85f), new Vector2(0.5f, 0.5f), new Vector2(50, 50), new Vector3(scalerTransform.rect.width / 2f, -scalerTransform.rect.height / 2f, 0));
                images[givenID][images[givenID].Count - 1].gameObject.SetActive(false);
